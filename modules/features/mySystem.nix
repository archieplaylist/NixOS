  ...
  config.nixos.modules.base = { config, lib, ... }: {
    options.mySystem = {
      ...
      desktop = lib.mkOption {
        type = lib.types.enum [ "gnome" "niri" "xfce" "plasma" ];
        default = "gnome";
        description = "Desktop environment for this host: gnome, niri, xfce or plasma.";
      };
          ...
          options =
            let
              groups = {
                ...
                ai = "AI coding agents (pi-coding-agent, opencode2 + ponytail/caveman skills).";
              };
            in
            lib.mapAttrs
              (_name: description: {
