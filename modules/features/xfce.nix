# XFCE stack slot (split out of desktop.nix). Display manager: LightDM. Unlike
# GNOME/Plasma (Wayland), XFCE is X11-based, so it relies on the shared
# `services.xserver` block from the `desktop` slot. The core session (xfwm4,
# xfdesktop, xfce4-panel, xfce4-session) comes from
# services.xserver.desktopManager.xfce; user-facing config (single bottom
# panel, window theme, xsettings) is shipped as system-wide xfconf defaults
# below. The panel XML is the user's own working config (a single bottom panel
# with Whisker menu) — the one layout that finally stopped the "(null)" plugin
# dialog. The extra packages are the thin apps the stock session doesn't ship
# (mirroring Plasma's dolphin/konsole).
#
...
'';
      ...
      # keyboard-shortcuts channel as a system default: xfce4-popup-whiskermenu
      # pops the menu (needs plugin-4 whiskermenu in the panel above).
